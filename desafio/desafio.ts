...
type Pokemon = {
  ...
  const returnByName = (name: string) =>
    pokemons.find(pokemonName => pokemonName.name == name);
 
 
 //console.log(returnByName("venusaur"))
 
 
 const returnByType = (type: PokemonTypes) =>
     pokemons.filter((pokemon) => pokemon.type.includes(type));
 
//console.log( returnByType("electric"))

 const translatePokemons = () =>
  pokemons.map((pokemon) => ({
    ...pokemon,
    type: pokemon.type.map((type: PokemonTypes) => typesArray[type]),
  }));
